...
async function findComment(playerId) {
    const response = await
        fetch('/fetchComment?id=' + playerId);
    return await
        response.json();
}
...
async function findBatterRecord(playerId) {
    const response = await
        fetch('/batterRecord?playerId=' + playerId);
    return await
        response.json();
}
...
async function findPitcherPerformance(playerId) {
    const response = await
        fetch('/pitcherPerformance?playerId=' + playerId);
    return await
        response.json();
}
...
async function findDefensivePerformance(playerId) {
    const response = await
        fetch('/defensivePerformance?playerId=' + playerId);
    return await
        response.json();
}
...
function pageReset() {
    ...
    const removeComments = Array.from(document.getElementById('comment').getElementsByTagName('p'))
    ...
    const removeBatterRecordTable = Array.from(document.getElementById('batterRecordTableBody').getElementsByTagName('td'))
    ...
    const removePitcherPerformanceTable = Array.from(document.getElementById('pitcherPerformanceTableBody').getElementsByTagName('td'))
    for (let removeTarget of removePitcherPerformanceTable) {
        removeTarget.remove()
    }

    const removeDefensivePerformanceTable = Array.from(document.getElementById('defensivePerformanceTable').getElementsByTagName('table'))
...
async function onClick() {
    ...
    try {
        ...
        document.getElementById('number').innerText = '背番号：　' + player.backNumber + '番';
        document.getElementById('name').innerText = '名前：　' + player.name;
        document.getElementById('position').innerText = 'ポジション：　' + player.position;
        document.getElementById('leftAndRight').innerText = '投 / 打：　' + player.leftAndRight;
        ...
        // 打撃成績表作成
        document.getElementById("batterRecordTable").classList.add("hide");
        if (player.position === '投手' || player.position === '捕手' || player.position === '内野手' || player.position === '外野手') {
            ...
            if (batterRecord.game) {
                ...
                for (let batterItem = 1; batterItem <= (Object.keys(batterRecord).length - 2); batterItem++) {
        ...
        if (player.position === '投手') {
            ...
            if (pitcherPerformance.pitched) {
                ...
                document.getElementById("pitcherPerformanceTable").classList.remove("hide");
                // idとplayerIdの2項目を除いて項目の回数for文を回す
                for (let pitcherItem = 1; pitcherItem <= (Object.keys(pitcherPerformance).length - 2); pitcherItem++) {
                    const td = document.createElement('td')
                    switch (pitcherItem) {
                        case pitched:
                    ...
                    document.getElementById('pitcherPerformanceTableBody').appendChild(td)
                }
            }
        }
        ...
        if (player.position === '投手' || player.position === '捕手' || player.position === '内野手' || player.position === '外野手') {
            ...
            if (defensivePerformance[0].defensivePosition) {
                for (let i = 0; i < defensivePerformance.length; i++) {
                    ...
                    document.getElementById('defensivePerformanceTableConteiner').appendChild(table)

                    const caption = document.createElement('caption')
                    caption.innerText = '守備成績(2021) 【' + defensivePerformance[i].defensivePosition + '】'
                    document.getElementById('defensivePerformance' + i).appendChild(caption)

                    const thead = document.createElement('thead')
                    thead.id = 'defensivePerformanceTableHead' + i
                    document.getElementById('defensivePerformance' + i).appendChild(thead)

                    if (defensivePerformance[i].defensivePosition === '捕手') {
                        // idとplayerIdとpositionの3項目を除いて項目の回数for文を回す
                        for (let defensiveTh = 1; defensiveTh <= (Object.keys(defensivePerformance[i]).length - 3); defensiveTh++) {
                            ...
                            document.getElementById('defensivePerformanceTableHead' + i).appendChild(th)
                        }
                    } else {
                        // idとplayerIdとpositionとpassedBallの4項目を除いて項目の回数for文を回す
                        for (let defensiveTh = 1; defensiveTh <= (Object.keys(defensivePerformance[i]).length - 4); defensiveTh++) {
                            const th = document.createElement('th')
                            let game = 1;
                            let putOut = 2;
                            let killing = 3;
                    ...
                    document.getElementById('defensivePerformance' + i).appendChild(tbody)

                    if (defensivePerformance[i].defensivePosition === '捕手') {
                        // idとplayerIdとpositionの3項目を除いて項目の回数for文を回す
                        for (let defensiveTd = 1; defensiveTd <= (Object.keys(defensivePerformance[i]).length - 3); defensiveTd++) {
                            const td = document.createElement('td')
                            let game = 1;
                            let putOut = 2;
                            let killing = 3;
                            ...
                            document.getElementById('defensivePerformanceTableBody' + i).appendChild(td)
                        }
                    } else {
                        // idとplayerIdとpositionとpassedBallの4項目を除いて項目の回数for文を回す
                        for (let defensiveTd = 1; defensiveTd <= (Object.keys(defensivePerformance[i]).length - 4); defensiveTd++) {
                            const td = document.createElement('td')
                            let game = 1;
                            let putOut = 2;
                            let killing = 3;
